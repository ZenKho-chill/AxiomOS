...
- Không cần ADR riêng cho implementation tối thiểu này vì không thay đổi boot protocol, ABI kernel hoặc kiến trúc renderer dài hạn.
- Quyết định đã chốt cho bản đầu: dùng glyph bitmap 5x7 nhúng tĩnh, scale 2x, clear màn hình khi hết dòng thay vì scrolling đầy đủ.
...
- `make image` build thành công trong WSL Ubuntu với image mặc định.
- QEMU headless xác nhận serial output không hồi quy:
...
- QEMU GTK/WSLg screenshot xác nhận framebuffer hiển thị boot sequence, gồm dòng `[AXIOMOS] Kernel started`.
- `KERNEL_FEATURES=panic-test bash scripts/build-image.sh` build thành công không warning.
- QEMU panic-test xác nhận panic handler ghi ra serial:

```text
[AXIOMOS PANIC] panicked at kernel/src/main.rs:68:5:
Spec 003 framebuffer panic test
```

- QEMU panic-test screenshot xác nhận panic message cũng được vẽ lên framebuffer.
