...
- Cần ADR cho chiến lược heap allocator nếu chọn crate hoặc thuật toán không tầm thường.

## Public interfaces

```rust
pub fn init_memory(memory_map: LimineMemoryMap) -> Result<MemoryStats, MemoryError>;
...
- `MemoryStats`: tổng usable, reserved, allocated, free.
- `MemoryError`: lỗi memory map, allocator hoặc paging.
