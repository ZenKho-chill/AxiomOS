...
- **Ngăn xếp (Userspace Stack)**: Đặt ở vùng nhớ cao của userspace canonical address. Điểm bắt đầu (stack pointer ban đầu `rsp`) sẽ là `0x00007FFFFFFFF000` (được căn lề trang). Kích thước stack ban đầu là 16 KiB (4 trang ảo, từ `0x00007FFFFFFF3000` đến `0x00007FFFFFFFF000`).
...
- **Linker Script Userspace**: Linker script cho các ứng dụng userspace phải chỉ định địa chỉ load tại `0x400000`.
